...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  test "that creating a friendship based on user id and friend id works" do
    ...
    context "#mutual_friendship" do
     setup do
        ...
        @friendship1 = users(:john).user_friendships.where(friend_id: users(:mike).id).first
        @friendship2 = users(:mike).user_friendships.where(friend_id: users(:john).id).first
      end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:john), users(:mike)
      assert !uf.save
    end
